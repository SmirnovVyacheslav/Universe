...
namespace engine {
    ...
    template <typename T> array<T>::array(int_32 size) {
        if (size < 0) {
            throw std::invalid_argument("Negative array size");
        }
        data.reserve(static_cast<size_t>(size));
    }
    template<typename T> int_32 array<T>::size() const {
    ...
    template<typename T> int_32 array<T>::find_index(const T& item) {
    ...
    template <typename T> T& array<T>::operator[](const int_32 index) {
        return data[(size() + index % size()) % size()];
    }
    template <typename T> const T& array<T>::operator[](const int_32 index) const {
        return data[(size() + index % size()) % size()];
    }
}
