...
namespace engine {
    ...
    void resource::load() {
        ...
        for (const auto& resource_file : std::filesystem::directory_iterator(config::resource()->model_path.s_str())) {
            string file_name = resource_file.path().filename().u8string();
            model_map.add(file_name, lead_ptr<model>());
            model_map[file_name].initialize();

            model_file.open(resource_file);
